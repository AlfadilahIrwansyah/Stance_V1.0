...
class ItemRegController extends Controller
{
    ...
    protected function NewItem(Request $request)
    {
        $newItem = RefItem::create([
            'ITEM_CODE' => $request->input('item_code')['item_code'],
            'ITEM_NAME' => $request->input('item_name')['item_name'],
            'REF_CATEGORY_ID' => $request->input('item_category')['item_category'],
            'STOCK' => $request->input('item_stock')['item_stock'],
            'BUY_PRICE_AMT' => $request->input('item_buy_price')['item_buy_price'],
            'SELL_PRICE_AMT' => $request->input('item_sell_price')['item_sell_price'],
            'IS_SELL' => $request->input('item_active')['item_active']
        ]);

        $newItem->save();
        return redirect()->route('addItem')->with('success', 'Add item successfully!');
    }
}
