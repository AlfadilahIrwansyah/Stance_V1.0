...
class SalesController extends Controller
{
    ...
    public function getTotalTransactionDetail($date = null)
    {
        ...
        $dataGrid = SalesTrxH::selectRaw('MONTH(TRX_DATE) as BULAN, TOTAL_TRX_AMT, COUNT(1) as TRANSAKSI, (ITEM_AMT * BUY_PRICE_AMT) as KEUNTUNGAN')
            ...
            ->selectSub(function ($query) {
                $query->from('sales_trx_h')
                    ...
                    ->join('ref_item', 'sales_trx_d.ref_item_id', '=', 'ref_item.ref_item_id')
                    ->groupBy('TOTAL_TRX_AMT', 'TRX_DATE', 'ITEM_AMT', 'BUY_PRICE_AMT')
                    ->selectRaw('MONTH(TRX_DATE) as BULAN, TOTAL_TRX_AMT, COUNT(1) as TRANSAKSI, (ITEM_AMT * BUY_PRICE_AMT) as KEUNTUNGAN');
            }, 'Y')
            ->groupBy('BULAN', 'TOTAL_TRX_AMT')
            ->select('BULAN', 'TOTAL_TRX_AMT', DB::raw('SUM(TRANSAKSI) as TOTAL_TRANSAKSI'), DB::raw('SUM(KEUNTUNGAN) as TOTAL_KEUNTUNGAN'))
            ->get();
