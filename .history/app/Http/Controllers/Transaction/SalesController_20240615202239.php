...
class SalesController extends Controller
{
    ...
    {
        $results = DB::table('REF_ITEM as RI')
            ->join('REF_CATEGORY as RC', 'RI.REF_CATEGORY_ID', '=', 'RC.REF_CATEGORY_ID')
            ->join(
                DB::raw('(SELECT RI.REF_CATEGORY_ID, SUM(STD.ITEM_AMT) AS TOTAL_ITEM_TERJUAL, SUM(STD.TOTAL_PRICE_AMT) AS JUMMLAH_NOMINAL_TERJUAL, STH.TRX_DATE
                     ...
                     GROUP BY RI.REF_CATEGORY_ID, STH.TRX_DATE) AS IC'),
                'IC.REF_CATEGORY_ID',
                '=',
                'RC.REF_CATEGORY_ID'
            )
            ->selectRaw('RC.CATEGORY_NAME, SUM(IC.TOTAL_ITEM_TERJUAL) AS TOTAL_ITEM_TERJUAL, SUM(IC.JUMMLAH_NOMINAL_TERJUAL) AS JUMMLAH_NOMINAL_TERJUAL, IC.TRX_DATE')
            ->groupBy('RC.CATEGORY_NAME', 'IC.TRX_DATE')
            ->get();
        dd($results);
    }
