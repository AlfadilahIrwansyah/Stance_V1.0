...
class SalesController extends Controller
{
    ...
    #endregion
    #region Sales by Month and Category
    ...
    private function getTotalTransactionDetail($dateStart = null, $dateEnd = null)
    {
        $subquery = DB::table('SALES_TRX_H as H')
            ...
            ->selectRaw('MONTH(H.TRX_DATE) as BULAN, YEAR(H.TRX_DATE) AS TAHUN ,SUM(D.TOTAL_PRICE_AMT) as TOTAL_OMSET, COUNT(1) as TRANSAKSI, SUM((D.ITEM_AMT * RI.SELL_PRICE_AMT) - (D.ITEM_AMT * RI.BUY_PRICE_AMT)) as KEUNTUNGAN, SUM(ITEM_AMT) AS TOTAL_ITEM')
            ->groupby(DB::raw('MONTH(H.TRX_DATE)'), DB::raw('YEAR(H.TRX_DATE)'));

        $dataGrid = DB::table(DB::raw("({$subquery->toSql()}) as Y"))
            ->mergeBindings($subquery)
            ->select('Y.BULAN', 'Y.TAHUN', 'Y.TOTAL_OMSET', DB::raw('SUM(Y.TRANSAKSI) as TOTAL_TRANSAKSI'), 'KEUNTUNGAN', 'TOTAL_ITEM');
        ...
        if ($dateStart !== null && $dateEnd !== null) {
            ...
            $dataGrid->whereRaw(
                'BULAN BETWEEN ? AND ? AND TAHUN BETWEEN ? AND ?',
        ...
        $dataGrid
            ->groupBy('Y.BULAN', 'Y.TAHUN', 'Y.TOTAL_OMSET', 'TOTAL_ITEM', 'KEUNTUNGAN')
            ->orderBy('Y.BULAN');
    ...
    private function getCategorySold($dateStart = null, $dateEnd = null)
    {
        ...
        $CategoryDataSold = DB::table('REF_CATEGORY as RI')
    ...
    #endregion
    #region Sales By User
    private function getUserPersonalSales($dateStart = null, $dateEnd = null)
    {
        $userSales = SalesTrxH::selectRaw('REF_USER.name as USER_NAME, SALES_TRX_H.TOTAL_TRX_AMT, TOTAL_ITEM')
        ...
        $dataSalesUser = $userSalesGrouped->map(function ($sales, $userName) {
            return [
                'Nama' => $userName,
                'TotalTrx' => $sales->sum('TOTAL_TRX_AMT'),
                'TotalItem' => $sales->sum('TOTAL_ITEM'),
            ];
        });
        $dataSalesUser = $dataSalesUser->values()->all();
        return $dataSalesUser;
    }

    public function showPeronalSales(UserSalesChart $UserSalesChart)
    {
        $datetime = Carbon::now()->format('Y-m');
        $dataSalesUser = $this->getUserPersonalSales();
        $userChart = $UserSalesChart->UserChart($dataSalesUser);
        return view('Transaction.SalesTrx.SalesInfo.SalesPersonal', [
            'datetime' => $datetime,
            'dataSaleUser' => $dataSalesUser,
            'userChart' => $userChart
        ]);
    }

    public function showPeronalSalesByDate(UserSalesChart $UserSalesChart, Request $request)
    {
        $datetime = Carbon::now()->format('Y-m');
        $periodStart = $request->input('periodStart', $datetime);
        $periodEnd = $request->input('periodEnd', $datetime);
        $dataSalesUser = $this->getUserPersonalSales($periodStart, $periodEnd);
        $userChart = $UserSalesChart->UserChart($dataSalesUser);
        if ($periodEnd < $periodStart || $periodStart > $periodEnd) {
            return back()->with('error', 'Input tanggal periode tidak sesuai');
        }
