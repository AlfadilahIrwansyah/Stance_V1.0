...
class SalesController extends Controller
{
    ...
    {
        if ($date !== null) {
            $items = RefItem::scopefilterDateItem($date)->get();
        } else {
            $items = RefItem::all();
        }
        $outcome = 0;
    ...
    {
        if ($date !== null) {
            $items = SalesTrxH::scopeFilterDateSales($date)->get();
        } else {
            $items = SalesTrxH::all();
        }
        $Turnover = 0;
    ...
    public function getTotalTransactionDetail($date)
    {
        if ($date !== null) {
            $salesTrxH = SalesTrxH::scopeFilterDateSales($date)->get();
        } else {
