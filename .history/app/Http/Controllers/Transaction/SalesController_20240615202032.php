...
class SalesController extends Controller
{
    ...
    {
        $results = DB::table(DB::raw('(SELECT MONTH(H.TRX_DATE) AS BULAN, H.TOTAL_TRX_AMT AS TOTAL_OMSET, COUNT(1) AS TRANSAKSI,
                                              ((D.ITEM_AMT * RI.SELL_PRICE_AMT) - (D.ITEM_AMT * RI.BUY_PRICE_AMT)) AS KEUNTUNGAN
                                       FROM SALES_TRX_H H
                                       INNER JOIN SALES_TRX_D D ON H.SALES_TRX_H_ID = D.SALES_TRX_H_ID
                                       INNER JOIN REF_ITEM RI ON D.REF_ITEM_ID = RI.REF_ITEM_ID
                                       GROUP BY H.TOTAL_TRX_AMT, H.TRX_DATE, D.ITEM_AMT, RI.BUY_PRICE_AMT, RI.SELL_PRICE_AMT) AS subquery'))
            ->selectRaw('BULAN, TOTAL_OMSET, SUM(TRANSAKSI) AS TOTAL_TRANSAKSI, SUM(KEUNTUNGAN) AS KEUNTUNGAN')
