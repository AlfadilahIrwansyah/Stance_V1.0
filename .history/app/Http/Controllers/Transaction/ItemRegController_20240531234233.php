...
class ItemRegController extends Controller
{
    ...
    public function update(Request $request, $refItemId)
    {
        $item = RefItem::findOrFail($id);

        $rules = [
            'item_code' => 'required|string|max:255|unique:ref_item,item_code,' . $item->id,
            'item_name' => 'required|string|max:255',
            'item_category' => 'required|integer',
            'item_desc' => 'required',
            'item_buy_price' => 'required|regex:/^\d+(\.\d{2})?$/',
        ...
        $messages = [
            ...
            'item_desc.required' => 'Deskripsi Barang harus diisi.',
            'item_buy_price.required' => 'Harga Beli harus diisi.',
            'item_buy_price.regex' => 'Harga Beli harus berupa angka dengan format yang benar.',
            'item_sell_price.regex' => 'Harga Jual harus berupa angka dengan format yang benar.',
            'item_stock.required' => 'Stok Barang harus diisi.',
            'item_stock.integer' => 'Stok Barang harus berupa angka.',
        ];
        ...
        $item->item_buy_price = $request->item_buy_price;
        $item->item_sell_price = $request->item_sell_price;
        $item->item_stock = $request->item_stock;
        $item->item_active = $request->has('item_active') ? 1 : 0;
        $item->save();
