...
class SalesController extends Controller
{
    ...
    private function getTotalTransactionDetail($dateStart = null, $dateEnd = null)
    {
        $subquery = DB::table('SALES_TRX_H as H')
            ...
            ->selectRaw('MONTH(H.TRX_DATE) BULAN WITH(NOLOCK), SUM(D.TOTAL_PRICE_AMT) TOTAL_OMSET WITH(NOLOCK), COUNT(1) as TRANSAKSI, SUM((D.ITEM_AMT * RI.SELL_PRICE_AMT) - (D.ITEM_AMT * RI.BUY_PRICE_AMT)) KEUNTUNGAN WITH(NOLOCK), SUM(ITEM_AMT) TOTAL_ITEM WITH(NOLOCK), TRX_DATE WITH(NOLOCK)')
            ->groupBy('H.TRX_DATE');
