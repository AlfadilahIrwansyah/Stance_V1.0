...
class SalesController extends Controller
{
    ...
    public function getTotalTransactionDetail($date = null)
    {
        ...
        $subquery = DB::table('SALES_TRX_H as H')
            ...
            ->selectRaw('MONTH(H.TRX_DATE) as BULAN, H.TOTAL_TRX_AMT as TOTAL_OMSET, COUNT(1) as TRANSAKSI, SUM(D.ITEM_AMT * D.BUY_PRICE_AMT) as KEUNTUNGAN')
            ->groupBy('H.TOTAL_TRX_AMT', 'H.TRX_DATE', 'D.ITEM_AMT', 'D.BUY_PRICE_AMT');

        $results = DB::table(DB::raw("({$subquery->toSql()}) as Y"))
            ->mergeBindings($subquery->getQuery())
            ->select('BULAN', 'TOTAL_OMSET', DB::raw('SUM(Y.TRANSAKSI) as TOTAL_TRANSAKSI'), DB::raw('SUM(Y.KEUNTUNGAN) as TOTAL_KEUNTUNGAN'))
