...
class SalesController extends Controller
{
    ...
    private function getTotalTransactionDetail($dateStart = null, $dateEnd = null)
    {
        $subquery = DB::table('SALES_TRX_H as H')
            ...
            ->selectRaw('MONTH(H.TRX_DATE) as BULAN, SUM(H.TOTAL_TRX_AMT) as TOTAL_OMSET, COUNT(1) as TRANSAKSI, SUM((D.ITEM_AMT * RI.SELL_PRICE_AMT) - (D.ITEM_AMT * RI.BUY_PRICE_AMT)) as KEUNTUNGAN, SUM(TOTAL_ITEM) AS TOTAL_ITEM, TRX_DATE')
            ->groupBy('H.TOTAL_TRX_AMT', 'H.TRX_DATE', 'RI.SELL_PRICE_AMT', 'TRX_DATE', 'RI.BUY_PRICE_AMT');

        $dataGrid = DB::table(DB::raw("({$subquery->toSql()}) as Y"))
            ->mergeBindings($subquery)
            ->select('Y.BULAN', 'Y.TOTAL_OMSET', DB::raw('SUM(Y.TRANSAKSI) as TOTAL_TRANSAKSI'), DB::raw('SUM(Y.KEUNTUNGAN) as TOTAL_KEUNTUNGAN'), 'TOTAL_ITEM');
        $Outcome = 0;
        if ($dateStart !== null && $dateEnd !== null) {
        ...
        $dataGrid->groupBy('Y.BULAN', 'Y.TOTAL_OMSET', 'TOTAL_ITEM')
            ->orderBy('Y.BULAN');
