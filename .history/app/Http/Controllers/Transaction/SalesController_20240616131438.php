...
class SalesController extends Controller
{
    ...
    private function getTotalTransactionDetail($dateStart = null, $dateEnd = null)
    {
        $subquery = DB::table('SALES_TRX_H as H')
            ...
            ->selectRaw('MONTH(H.TRX_DATE) as BULAN, SUM(D.TOTAL_PRICE_AMT) as TOTAL_OMSET, COUNT(1) as TRANSAKSI, SUM((D.ITEM_AMT * RI.SELL_PRICE_AMT) - (D.ITEM_AMT * RI.BUY_PRICE_AMT)) as KEUNTUNGAN, SUM(ITEM_AMT) AS TOTAL_ITEM')
            ->groupby(DB::raw('MONTH(H.TRX_DATE'));
        ...
        if ($dateStart !== null && $dateEnd !== null) {
            ...
            // );
            $startYear = Carbon::parse($dateStart)->year;
            $startMonth = Carbon::parse($dateStart)->month;

            $endYear = Carbon::parse($dateEnd)->year;
            $endMonth = Carbon::parse($dateEnd)->month;

            $subquery->whereBetween(DB::raw('DATE(H.TRX_DATE)'), [$dateStart, $dateEnd]);
        } else {
    ...
    private function getUserPersonalSales(){
        $userSales = SalesTrxH::selectRaw('REF_USER.name as user_name, SALES_TRX_H.total_trx_amt, TOTAL_ITEM')
            ...
            ->get();
        $userSalesGrouped = $userSales->groupBy('user_name');
    ...
    public function showPeronalSalesByDate(){
