...
class Barchart extends LarapexChart
{
    ...
    public function buildHomeLinechart(): \ArielMejiaDev\LarapexCharts\LineChart
    {
        ...
        $chart = $dom->getElementById('homeChart');
        $classAttr = $chart->getAttribute('class');
        if (condition) {
            $chart->setFontColor('#ffffff');
        }

        if (strpos($classAttr, 'bg-dark') !== false) {
            echo "Element has 'bg-dark' class: " . $div->nodeValue . "\n";
        } else {
            echo "Element does not have 'bg-dark' class: " . $div->nodeValue . "\n";
        }
