...
class Barchart
{
    ...
    public function buildHomeLinechart(): \ArielMejiaDev\LarapexCharts\LineChart
    {
        ...
        $query = DB::table('SALES_DATA_TRX_H')
        ->join('SALES_DATA_TRX_D', 'SALES_DATA_TRX_H.SALES_DATA_TRX_H_ID', '=', 'SALES_DATA_TRX_D.SALES_DATA_TRX_H_ID')
            ->selectRaw('YEAR(date_sale) as year, MONTH(date_sale) as month, SUM(amount) as total_sales')
            ->groupBy((DB::raw('YEAR(date_sale)')), (DB::raw('MONTH(date_sale)')));
