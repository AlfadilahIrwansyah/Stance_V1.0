...
class Barchart extends LarapexChart
{
    ...
    public function buildHomeLinechart(bool $isDark = true): \ArielMejiaDev\LarapexCharts\LineChart
    {
        $dom = new DOMDocument();
        ...
        $query = DB::table('SALES_TRX_H')
            ...
            ->selectRaw('YEAR(TRX_DATE) as year, MONTH(TRX_DATE) as month, TOTAL_TRX_AMT as total_sales');
        ...
        $chartTheme = $dom->getElementById('homeChart');
        $classAttr = $chartTheme->getAttribute('class');
        if (strpos($classAttr, 'bg-dark') !== false) {
            $chart->setFontColor('#ffffff');
        } else {
