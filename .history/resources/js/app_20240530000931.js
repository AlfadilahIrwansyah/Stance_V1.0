...
$(document).ready(function() {
    ...
    var hasBgDark = $('#homeChart').hasClass('bg-dark');
        var fontColor = hasBgDark ? '#ffffff' : '#000000';

        $.ajax({
            url: '/get-home-linechart',
            method: 'GET',
            data: { fontColor: fontColor },
            success: function(response) {
                // Render the chart with the response data
                $('#homeChart').html(response.chartHtml);
            }
        });
});
