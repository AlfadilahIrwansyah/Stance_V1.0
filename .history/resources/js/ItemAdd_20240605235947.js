$(function() {
    ...
    function formatCurrency(value, currency = false) {
        var ValueFormatted = parseFloat(itemTotal).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')
        ...
        if (currency) {
        console.log('formatted ' + ValueFormatted.replace(/\d(?=(\d{3})+\.)/g, '$&,'));
    ...
    $('#saveItemButton').on('click', function() {
        console.log('test 1');
            ...
            console.log('3');
            console.log(formatCurrency(itemTotal, true));
            var newRow = '<tr>' +
            '<td>' + itemCode + '</td>' +
            '<td>' + itemName + '</td>' +
            '<td>' + itemQty + '</td>' +
