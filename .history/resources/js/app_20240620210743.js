...
$(function() {
    ...
    var sidebarLogo = $('.sidebar-header').find('.sidebar-logo').detach();
    ...
    $(document).on('click',function(event) {
        if (!sidebar.is(event.target) && !sidebar.has(event.target).length && !toggleBtn.is(event.target)) {
            ...
            if ($('.sidebar-header').find('.sidebar-logo').length === 0) {
                    $('.sidebar-header').prepend(sidebarLogo);
                } else {
                    sidebarLogo = $('.sidebar-header').find('.sidebar-logo').detach();
            }
        }
    });
