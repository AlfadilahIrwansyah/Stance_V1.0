            ...
            <tbody class="table-info">
                @foreach ($RefItem as $item)
                    <tr scope="row" id="rowBarang">
                        ...
                        <td class="text-center">{{ $item->REF_CATEGORY->CATEGORY_NAME }}</td>
                        <td class="text-center">{{ format_currency($item->SELL_PRICE_AMT, true) }}</td>
                        <td class="text-center">{{ DateFormatter($item->BUY_DATE) }}</td>
                        <td>
                            @if ($item->IS_SELL)
