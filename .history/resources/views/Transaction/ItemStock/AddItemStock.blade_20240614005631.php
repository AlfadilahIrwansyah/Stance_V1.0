        ...
        <form action="{{ isset($item) ? route('updateItem', $item->REF_ITEM_ID) : route('NewItem') }}" method="POST">
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    ...
                    <input type="date" name="item_sell_price" id="item_sell_price"
                        class="form-control @error('item_sell_price') is-invalid @enderror"
                        value="{{ old('item_sell_price', isset($item) ? format_currency($item->SELL_PRICE_AMT) : '0.00') }}"
                        autocomplete="item_sell_price">
                    <input type="date" class="form-control @error('buy_date') is-invalid @enderror" id="buy_date"
                        name="buy_date" autocomplete="item_sell_price"
                        value="{{ old('buy_date', isset($item) ? $item->BUY_DATE : Carbon::now()->format('Y-M-d')) }}">
