...
@section('content')
    ...
    <script>
        function checkSellPrice() {
            const sellPrice = document.getElementById('item_sell_price').value;
            const itemActive = document.getElementById('item_active');

            if (sellPrice !== '0.00' && sellPrice !== '') {
                itemActive.disabled = false;
            } else {
                itemActive.disabled = true;
                itemActive.checked = false;
            }
        }

        document.addEventListener('DOMContentLoaded', function() {
            checkSellPrice(); // Initial check on page load
        });
    </script>
