...
@section('content')
    <div class="container">
        ...
        <form action="{{ route('AddTrx') }}" method="POST">
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    ...
                    <input type="text" name="user_cashier" id="user_cashier" class="form-control"
                        value="{{ old('user_cashier', $TransData->user->name) }}" required autocomplete="user_cashier" disabled>
                        <input type="hidden" name="user_cashier" id="user_cashier" value="{{ old('user_cashier', $TransData->user->id) }}">
            ...
            <div class="d-flex align-items-center justify-content-end">
                <button type="submit" class="btn btn-danger align-items-end justify-content-end me-2"
                    style="width: 100px;">
                    {{ __('Cancel') }}
                </button>
                <button type="submit" class="btn btn-primary align-items-end justify-content-end me-2"
                    style="width: 100px;">
                    {{ __('Print') }}
                </button>
                                ...
                                <select class="form-control" id="itemCode" style="width: 100%;" required>
                                    ...
                                    @foreach ($refItem as $item)
                                        <option value="{{ $item->ITEM_CODE }}" data-name="{{ $item->ITEM_NAME }}"
                                            ...
                                            data-itemid="{{ $item->REF_ITEM_ID }}">{{ $item->ITEM_NAME }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
