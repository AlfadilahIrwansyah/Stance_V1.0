...
@section('content')
    <div class="container">
        ...
        <div id="gridDetail">
            ...
            <div id="itemGrid">
                <table class="table table-bordered table-striped table-hover border-dark align-middle rounded-1"
                    id="itemsTable" style="margin-top: 10px">
                    ...
                    <tbody class="table-info" id="itemsTable">
                        @foreach ($salesTrxD as $item)
                            <tr>
                                ...
                                <td>{{ $item->ITEM_AMT }}</td>
                                <td>{{ format_currency($item->TOTAL_PRICE_AMT, true) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                <div id="TotalHarga" class="d-flex flex-column align-items-end justify-content-end">
                    <div class="d-flex justify-content-center">
                        <p class="text text-dark fw-bolder text-right">
                            {{ __('Total : ') }}<span
                                id="total">{{ format_currency($salesTrxH->TOTAL_TRX_AMT, true) }}</span>
                        </p>
                    </div>
                </div>
            </div>
