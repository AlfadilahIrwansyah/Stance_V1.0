...
@section('content')
    <div class="container">
        ...
        <div id="itemGrid">
            <button type="submit" class="btn btn-primary align-items-end justify-content-between"" style="width: 100px;">
                {{ __('Add Item') }}
            </button>
                ...
                <tbody class="table-info">
                    @foreach ($itemData as $item)
                        <tr scope="row" id="rowBarang">
                            ...
                            <td>{{ $item->ItemName }}</td>
                            <td>{{ $item->Qty }}</td>
                            <td>{{ 'Rp ' . format_currency($item->Price) }}</td>
                            <td>{{ 'Rp ' . format_currency($item->Price * $item->Qty) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
